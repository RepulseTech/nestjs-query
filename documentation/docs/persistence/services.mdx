...
`@repulsetech/nestjs-query-core` also provides a number of base `QueryService`s that can be used to create custom query services.
...
```ts title="todo-item.entity.ts"
import { Table, Column, Model, AllowNull, CreatedAt, UpdatedAt, PrimaryKey, AutoIncrement } from 'sequelize-typescript';
